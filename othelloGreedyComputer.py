...
def greedyComputerMode():
    nameInput = input("What is your name? ")
    quitFunction(nameInput)
    if (random.randint(1,1) == 1):  #When we are playing against the computer I want to randomize who will go first. Someitmes the computer will go first and sometimes the player will go first
        playerOne = nameInput
        playerTwo = "CPU"
    ...
    def turnOperations(flag):
        if (flag == True):
            pInfo = ['B','W',playerOne] #The first element is opColor, the second element is color, and the third element is the name of player
            return pInfo
        else:
            pInfo = ['W','B',playerTwo]
            return pInfo
    ...
    while (True):
        ...
        tokenFlips = theBoard.checkPoints(pInfo[0],pInfo[1])[1]
